...
export function BallotCard({ ballot, busy, owned, onOpen, onVote, onEdit, onDelete, onCloseBallot }: BallotCardProps) {
  ...
  return (
    <>
        ...
        <div className={styles.body}>
          <div className={styles.copy}>
            ...
            <button
              ...
              data-qa-author
              aria-label={t('ballots', 'publicAuthorRecord')}
            >
              <BallotMark
                icon={ballot.author.avatarIcon ?? 'CITIZEN'}
                color={ballot.author.avatarColor ?? 'NAVY'}
                size="small"
              />
      ...
      {authorOpen && (
        <PublicProfileDialog userId={ballot.author.id} onClose={() => setAuthorOpen(false)} />
      )}
    </>
  );
}
